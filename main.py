...
def main(args):
    ...
    print(f"Creating model: {args.mae_model}")
    ...
    if args.resume or os.path.exists(f'{args.output_dir}/{args.mae_model}_best_checkpoint.pth'):
        args.resume = f'{args.output_dir}/{args.mae_model}_best_checkpoint.pth'
    ...
    if args.eval:
        ...
        print(f"Evaluating model: {args.mae_model}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if max_accuracy < test_stats["acc1"]:
            ...
            if args.output_dir:
                ckpt_path = os.path.join(output_dir, f'{args.mae_model}_best_checkpoint.pth')
    ...
    if args.predict and utils.is_main_process():
        model_predict = MaskAutoEncoderClassify(
            args.mae_model,
            num_classes=args.nb_classes,
            mae_pretrained_path=args.mae_pretrained_path,
            args=args
        ...
        Predictor(model_predict, data_loader_val, f'{args.output_dir}/{args.mae_model}_best_checkpoint.pth', device)
        Plot_ROC(model_predict, data_loader_val, f'{args.output_dir}/{args.mae_model}_best_checkpoint.pth', device)

        if args.opt_auc:
            OptAUC(model_predict, data_loader_val, f'{args.output_dir}/{args.mae_model}_best_checkpoint.pth', device)
