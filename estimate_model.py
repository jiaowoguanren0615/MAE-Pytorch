...
def Plot_ROC(net: torch.nn.Module, val_loader: Iterable, save_name: str, device: torch.device):
    ...
    net.load_state_dict(torch.load(save_name, weights_only=True)['model'])
...
@torch.inference_mode()
def predict_single_image(model: torch.nn.Module, device: torch.device, save_name: str):
    """
    ...
    assert os.path.exists(save_name), "weight file dose not exist."
    model.load_state_dict(torch.load(save_name, weights_only=True, map_location=device)['model'])
